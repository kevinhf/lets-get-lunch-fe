describe('Recommendations List', () => {
    ...
    it('should not display a list of recommendations for an event that ' + 'has suggestLocations set to false', () => {
        ...
        cy.createEvent(eventName, 'Atlanta', {suggestLocations: false })
    ...
    it('should display a list of recommendations for an event that ' + 'has suggestLocations set to true', () => {
        ...
        cy.createEvent(eventName, 'Atlanta', {suggestLocations: true})
        .get('[data-test=events]').click()
        .url().should('include', '/events')
        .get('.event-title').contains(eventName).next().next().children().click()
        .url().should('include', '/event/')
    ...
    it('should display an error message for an event that\'s in the boonies', () => {
        ...
        cy.createEvent(eventName, 'Bayou LaBatre', {suggestLocations: true })
        .get('[data-test=events]').click()
        .url().should('include', '/events')
        .get('.event-title').contains(eventName).next().next().children().click()
        .url().should('include', '/event/')
