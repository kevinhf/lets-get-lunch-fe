describe('Event Subscribe', () => {
    ...
    it('should not display a subscribe button for the event creator', () => {
        ...
        .get('.cal-event .cal-event-title').should('have.text', eventName).click()
        .url().should('include', '/event/')
        .get('.event-subscribe').should('not.exist');
    });

    it('should update the member list when a user clicks the subscribe ' + 'and unsubscribe button', () => {
        cy.login(subscriber, 'foobar')
        ...
        .get('[data-test=unsubscriber]').should('have.text', 'Unsubscribe').click()
