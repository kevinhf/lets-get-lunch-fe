describe('Event Update', () => {
    ...
    beforeEach(() => {
        cy.signup().createEvent('Lunch', 'Atlanta').get('.cal-event .cal-event-title').should('have.text', 'Lunch').click()
        .url().should('include', '/event/').get('.event-name').should('have.text', 'Lunch');
    });

    it('should redirect to the event update page when ' + 'the edit link is clicked', () => {
        cy.get('.event-edit').click().url().should('include', '/update');
    });

    it('should display a success message for an event that ' + 'has successfully been updated', () => {
        cy.get('.end').invoke('text').then(end1 => {
