describe('Comments', () => {
    ...
    beforeEach(() => {
        cy.signup().createEvent('Lunch', 'Atlanta')
        .get('.cal-event .cal-event-title').should('have.text', 'Lunch').click().url().should('included', '/event/')
        .get('.event-name').should('have.text', 'Lunch');
    });

    it('should display a message that no comments exist for ' + 'an event with no comments', () => {
        cy.get('[data-test=no-comments]').should('be.visible');
    });

    it('should populate the comment view when a user submits a comment', () => {
        cy.get('textarea').type('My first comment')
        .get('button[type=submit]').click()
        .get('[data-test=no-comments]').should('not.exist')
