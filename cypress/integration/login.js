describe('Login', () => {
    ...
    before(() => {
        unqiue = 'uniqueUser';
        cy.signup(unique, 'password').get('[data-test=logout]').should('have.text', 'Logout').click();
    });

    it('should display an error message for an incorrect password', () => {

        cy.visit('/login').url().should('include', '/login')
            .get('#username').type(unqiue)
    ...
    it('should log in a user who deos exist redirecting ' + 'them to the dashboard', () => {
